...
exports.getPaytrToken = functions.https.onRequest(async (req, res) => {
  try {
    ...
    const hash_str = merchant_id + user_ip + merchant_oid + email + payment_amount + user_basket +
...
exports.paytrCallback = functions.https.onRequest(async (req, res) => {
  try {
    ...
    if (token !== hash) return res.status(400).send("INVALID_HASH");
